...
// Configure multer with file size limits only
// Configure multer with file size limits and file type validation
const upload = multer({
    storage: storage,
    limits: {
    ...
    fileFilter: (req, file, cb) => {
        ...
        // قبول جميع أنواع الملفات الشائعة
        const allowedMimes = [
            // صور
            'image/jpeg',
            'image/png',
            ...
            // مستندات
            'application/pdf',
            'application/msword',  // ملفات Word القديمة (doc)
            'application/vnd.openxmlformats-officedocument.wordprocessingml.document',  // ملفات Word الجديدة (docx)
            'application/vnd.ms-excel',
            'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
            'application/vnd.ms-powerpoint',
            'application/vnd.openxmlformats-officedocument.presentationml.presentation',
            'text/plain',  // ملفات النصوص (txt)
            'application/rtf',
            'application/zip',
            'application/x-zip-compressed'
        ];

        // فحص امتداد الملف للتعرف على أنواع إضافية
        const ext = path.extname(file.originalname).toLowerCase();
        const is3gp = ext === '.3gp' || ext === '.3gpp';
        const isTextFile = ext === '.txt';
        const isWordFile = ext === '.doc' || ext === '.docx';

        // قبول أي نوع ملف يبدأ بهذه البادئات أو له امتداد محدد
